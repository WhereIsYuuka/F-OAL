...
class CelebA(DatasetBase):
    def __init__(self, scenario, params):
        ...
        super(CelebA, self).__init__(dataset, scenario, num_tasks, params.num_runs, params)


    def download_load(self):
        if os.path.exists('./datasets/CelebA/CelebA_train_data.pkl') and os.path.exists('./datasets/CelebA/CelebA_train_label.pkl') and \
                os.path.exists('./datasets/CelebA/CelebA_test_data.pkl') and os.path.exists('./datasets/CelebA/CelebA_test_label.pkl'):
            with open('./datasets/CelebA/CelebA_train_data.pkl', 'rb') as f:
                self.train_data = pickle.load(f)
            with open('./datasets/CelebA/CelebA_train_label.pkl', 'rb') as f:
                self.train_label = pickle.load(f)
            with open('./datasets/CelebA/CelebA_test_data.pkl', 'rb') as f:
                self.test_data = pickle.load(f)
            with open('./datasets/CelebA/CelebA_test_label.pkl', 'rb') as f:
                self.test_label = pickle.load(f)
        else:
            train = datasets.CelebA(root=self.root, split='train', download=True)
            test = datasets.CelebA(root=self.root, split='test', download=True)
            self.train_data = []
            self.train_label = []
            self.test_data = []
            self.test_label = []
            ...
            with open('./datasets/CelebA/CelebA_train_data.pkl', 'wb') as f:
                pickle.dump(self.train_data, f)
            with open('./datasets/CelebA/CelebA_train_label.pkl', 'wb') as f:
            ...
            with open('./datasets/CelebA/CelebA_test_data.pkl', 'wb') as f:
                pickle.dump(self.test_data, f)
            with open('./datasets/CelebA/CelebA_test_label.pkl', 'wb') as f:
    ...
    def setup(self):
        if self.scenario == 'ni':
            ...
            self.task_labels = create_task_composition(class_nums=10177, num_tasks=self.task_nums, fixed_order=self.params.fix_order)
            self.test_set = []
