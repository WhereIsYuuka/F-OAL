...
class FLOWER102(DatasetBase):
    def __init__(self, scenario, params):
        ...
        super(FLOWER102, self).__init__(dataset, scenario, num_tasks, params.num_runs, params)


    def download_load(self):
        dataset_train = datasets.Flowers102(root=self.root, split='train', download=True)
        # self.train_data = dataset_train.data
        # self.train_label = np.array(dataset_train.target_transform)
        self.train_data, self.train_label = dataset_train
        dataset_test = datasets.Flowers102(root=self.root, split='test', download=True)
        # self.test_data = dataset_test
        # self.test_label = np.array(dataset_test.target_transform)
        self.test_data, self.test_label =dataset_test
