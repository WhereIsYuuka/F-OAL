...
class tiny(DatasetBase):
    def __init__(self, scenario, params):
        ...
        super(tiny, self).__init__(dataset, scenario, num_tasks, params.num_runs, params)


    def download_load(self):
        if os.path.exists('./datasets/tiny/tiny_train_data.pkl') and os.path.exists('./datasets/tiny/tiny_train_label.pkl') and \
                os.path.exists('./datasets/tiny/tiny_test_data.pkl') and os.path.exists('./datasets/tiny/tiny_test_label.pkl'):
            with open('./datasets/tiny/tiny_train_data.pkl', 'rb') as f:
                self.train_data = pickle.load(f)
            with open('./datasets/tiny/tiny_train_label.pkl', 'rb') as f:
                self.train_label = pickle.load(f)
            with open('./datasets/tiny/tiny_test_data.pkl', 'rb') as f:
                self.test_data = pickle.load(f)
            with open('./datasets/tiny/tiny_test_label.pkl', 'rb') as f:
                self.test_label = pickle.load(f)
        else:
            train = datasets.ImageFolder( './datasets/tinyimagenet/train')
            test = datasets.ImageFolder( './datasets/tinyimagenet/test')
            self.train_data = []
            self.train_label = []
            self.test_data = []
            self.test_label = []
            ...
            with open('./datasets/tiny/tiny_train_data.pkl', 'wb') as f:
                pickle.dump(self.train_data, f)
            with open('./datasets/tiny/tiny_train_label.pkl', 'wb') as f:
            ...
            with open('./datasets/tiny/tiny_test_data.pkl', 'wb') as f:
                pickle.dump(self.test_data, f)
            with open('./datasets/tiny/tiny_test_label.pkl', 'wb') as f:
    ...
    def setup(self):
        if self.scenario == 'ni':
            ...
            self.task_labels = create_task_composition(class_nums=200, num_tasks=self.task_nums, fixed_order=self.params.fix_order)
            self.test_set = []
