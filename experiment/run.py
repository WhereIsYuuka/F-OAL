...
def multiple_run(params, store=False, save_path=None):
    ...
    if params.online:
        ...
        with open(params.data + '_' + params.agent + '.csv', mode='a+', encoding="ISO-8859-1", newline='') as file:
            data = ('avg_end_acc',) + tuple(avg_end_acc) + ('-',) + ('avg_acc',) + tuple(avg_acc) + ('-',) + \
                   ('fog',) + tuple(avg_end_fgt) + ('-',) + ('time_total',) + (str(end - start),) + ('-',) + (
                   'train_time',) + (str(sum(trainingtime) / params.num_runs),)+('-',)+('test_time',)+(str(sum(testingtime) / params.num_runs),)+('-',)
            wr = csv.writer(file)
            wr.writerow(data)
        print('Records have been written')
    else:
        print('----------- Total {} run: {}s -----------'.format(params.num_runs, end - start))
